...
function styleForPose(sprite: PlayerSprite, pose: Pose, scale = 1): PlayerStyle {
...
const Player: React.FC<PlayerProps> = ({ character, pose, scale = 2 }) => {
  const sprite = sprites.get(character);

  if (!sprite) return null;

  return (
    <div className="ReflexDuel__Player" style={{ ...styleForPose(sprite, pose, scale) }} />
  );
};
